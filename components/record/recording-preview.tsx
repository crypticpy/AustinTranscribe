 ...
 *
 * Shows after recording is complete, allowing preview, save, and transcription.
 *
 * Features:
 ...
 * - Transcribe now (navigate to upload flow)
...
import { saveRecording, updateRecordingStatus } from "@/lib/db";
...
export interface RecordingPreviewProps {
  ...
  /** @deprecated No longer used - transcription happens inline */
  onTranscribe?: () => void;
}
...
export function RecordingPreview({
  ...
  const [savedRecordingId, setSavedRecordingId] = React.useState<number | null>(null);
  ...
  const transcription = useTranscriptionFlow({
    ...
    onComplete: async (transcriptId) => {
      // Update recording status to 'transcribed' and link to transcript
      if (savedRecordingId) {
        try {
          await updateRecordingStatus(savedRecordingId, "transcribed", transcriptId);
        } catch (error) {
          console.error("Failed to update recording status:", error);
        }
      }
      // Navigate to transcript analysis page after a brief delay
      setTimeout(() => {
        router.push(`/transcripts/${transcriptId}/analyze`);
      }, 1500);
    },
    onError: (error) => {
  ...
  /**
   * Handle transcribe now - saves first then starts transcription
   */
  const handleTranscribeNow = React.useCallback(async () => {
    try {
      ...
      const filename = recordingName.trim() || `recording-${new Date().toISOString().slice(0, 10)}`;
      await transcription.startTranscription(audioBlob, filename);
    } catch (error) {
      console.error("Failed to save/transcribe recording:", error);
      setIsSaving(false);
  ...
  }, [audioBlob, mode, duration, fileSize, mimeType, createdAt, recordingName, transcription]);
  ...
  const handleRetryTranscription = React.useCallback(() => {
    transcription.reset();
    const filename = recordingName.trim() || `recording-${new Date().toISOString().slice(0, 10)}`;
    transcription.startTranscription(audioBlob, filename);
  }, [transcription, recordingName, audioBlob]);
  ...
  // Show transcription progress when transcribing
  if (isTranscribing) {
    return (
      <TranscriptionProgress
        status={transcription.state.status}
        progress={transcription.state.progress}
        message={transcription.state.message}
        error={transcription.state.error?.message}
        onCancel={handleCancelTranscription}
        onRetry={handleRetryTranscription}
        onViewTranscript={
          transcription.state.transcriptId
            ? () => router.push(`/transcripts/${transcription.state.transcriptId}/analyze`)
            : undefined
        }
        onDone={handleCancelTranscription}
      />
    );
  }
