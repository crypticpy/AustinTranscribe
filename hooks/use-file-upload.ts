...
async function extractAudioMetadata(file: File): Promise<AudioMetadata> {
  ...
  // Return placeholder metadata - real duration will be available after conversion
  if (isVideo) {
    return {
      duration: 0, // Unknown until conversion to audio
    ...
    try {
      ...
      const transcriptResults = await Promise.all(
        filesToProcess.map(async (file, index) => {
          ...
          const partLabel = isMultiPart ? ` (part ${index + 1}/${totalParts})` : '';
