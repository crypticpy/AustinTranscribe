 ...
 * - Basic: Fast monolithic single-pass analysis (<30-60 sec) for short meetings
 * - Hybrid: Batched analysis with context (2-3 min) for medium-length meetings
 * - Advanced: Serial cascading analysis (4-5 min) for complex, long meetings
 *
 * Strategy Selection:
...
export const STRATEGY_TIME_ESTIMATES = {
  basic: { min: 30, max: 60 },      // Single API call
  hybrid: { min: 120, max: 180 },   // 3 batched API calls
  advanced: { min: 240, max: 300 }, // 9-10 sequential API calls
} as const;
...
export function recommendStrategy(transcriptText: string): StrategyRecommendation {
  ...
  if (tokens < STRATEGY_BREAKPOINTS.BASIC_THRESHOLD) {
    ...
    reasoning =
      ...
      'Basic analysis provides fast results (30-60 sec) with comprehensive coverage ' +
    ...
    alternatives = [
      {
        ...
        tradeoff: '2-3x slower but marginal quality improvement for short meetings',
      },
      {
        strategy: 'advanced',
        reason: 'Maximum quality with full contextual cascading',
        tradeoff: '4-5x slower with minimal benefit for short transcripts',
      },
    ];
    ...
    reasoning =
      ...
      'Hybrid analysis balances speed and quality (2-3 min) with contextual batching ' +
    ...
    alternatives = [
      ...
      },
      {
        ...
        tradeoff: '2x slower but provides deepest contextual understanding',
      },
    ];
  } else {
    ...
    reasoning =
      ...
      'Advanced analysis recommended for complex discussions (4-5 min processing). ' +
  ...
  if (selectedStrategy === 'advanced' && tokens < STRATEGY_BREAKPOINTS.BASIC_THRESHOLD) {
    return {
      ...
      warning:
        ...
        `(${tokens.toLocaleString()} tokens). This will take 4-5 minutes with minimal quality ` +
