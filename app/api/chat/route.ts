...
function truncateHistory(
  ...
  if (truncated.length < conversationHistory.length) {
    console.log(
      `[Chat] Truncated conversation history from ${conversationHistory.length} to ${truncated.length} messages ` +
      `to fit within ${maxTokens} token limit`
    );
  }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (tokenEstimate.totalTokens > deploymentInfo.tokenLimit) {
      console.error('[Chat] Token limit exceeded:', {
    ...
    try {
      ...
      console.error('[Chat] OpenAI API call failed:', error);
      console.error('[Chat] Full error details:', JSON.stringify(error, Object.getOwnPropertyNames(error), 2));
    ...
    if (!answer) {
      console.error('[Chat] No answer in OpenAI response:', completion);
      return chatErrorResponse(
        'api_failure',
